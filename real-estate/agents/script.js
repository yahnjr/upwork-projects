...
function loadAgents() {
    ...
    Promise.all([
        ...
        fetch('states.geojson').then(response => {
            if (!response.ok) throw new Error(`HTTP error! Status: ${response.status}`);
            return response.json();
        })
    ])
...
viewMode.addEventListener('click', () => {
    ...
    setTimeout(() => {
        const source = map.getSource('localAgents');
        if (source && map.getZoom() > 10) {
...
polygonMode.addEventListener('click', () => {
    ...
    selectedStates = null;
    map.setPaintProperty('states', 'fill-opacity', 0.0);
...
map.on('click', 'states', (e) => {
    ...
    fetch('real_estate_agents.geojson')
