...
function loadSocialMedia() {
    ...
    Promise.all([
        fetch('LocalSocialMedia.geojson').then(response => {
            if (!response.ok) throw new Error(`HTTP error! Status: ${response.status}`);
            return response.json();
        }),
        fetch('NationalAccounts.geojson').then(response => {
            if (!response.ok) throw new Error(`HTTP error! Status: ${response.status}`);
            return response.json();
        }),
        fetch('NationalAccountData.json').then(response => {
            if (!response.ok) throw new Error(`HTTP error! Status: ${response.status}`);
            return response.json();
        })
    ])
    .then(([localSocialData, nationalSocialData, nationalData]) => {
        ...
        map.on('click', 'clusters', (e) => {
            ...
            map.getSource('localSocial').getClusterExpansionZoom(
                clusterId,
                (err, zoom) => {
                    if (err) return;
