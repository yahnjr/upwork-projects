...
// Dependency manager to load external scripts and stylesheets in the correct order
const DependencyLoader = {
    ...
    loadAll: async function() {
      try {
        ...
        await Promise.all([
          this.loadStyle('https://api.mapbox.com/mapbox-gl-js/v2.9.1/mapbox-gl.css'),
        ...
        await this.loadScript('https://api.mapbox.com/mapbox-gl-js/v2.9.1/mapbox-gl.js');
  ...
  // Start loading dependencies and initialize the app when ready
  DependencyLoader.loadAll().then((success) => {
    if (success) {
      initializeApp();
    } else {
      document.getElementById('gpx-video-app-wrap').innerHTML = 
        '<div style="padding: 20px; text-align: center; color: red;">Failed to load required resources. Please check console for details.</div>';
    }
  });
